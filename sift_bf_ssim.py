...
def start(img1, img2):
    ...
    img2 = cv2.resize(img2, (int(width2 * w), int(height2 * h)), interpolation=cv2.INTER_AREA)
    ...
    img1 = cv2.resize(img1, (int(height1/10), int(width1/10)), interpolation=cv2.INTER_LANCZOS4)
    img2 = cv2.resize(img2, (int(height1/10), int(width1/10)), interpolation=cv2.INTER_LANCZOS4)
    ...
    res = cv2.drawMatches(img1, kp1, img2, kp2, [], None, flags=cv2.DRAW_MATCHES_FLAGS_NOT_DRAW_SINGLE_POINTS)
